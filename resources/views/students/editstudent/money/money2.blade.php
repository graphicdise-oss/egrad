    ...
    <div class="flex min-h-screen">
        ...
        <div class="p-8">
            ...
            <div class="bg-white p-6 shadow-md w-3/4 border">
                ...
                <h2 class="text-lg font-semibold mb-4">@lang('form.payment2_1')
                    <hr>
                </h2>
                ...
                @if($payments->isEmpty())
                    <p class="text-center text-gray-500">@lang('form.paymentno2')</p>
                @else
                    <div class="space-y-4">
                        @foreach($payments as $index => $payment)
                            <div class="row mb-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" class="form-control" value="{{ $payment->term ?? '-' }}" readonly>
                                </div>

                                <div class="col-md-3">
                                    <label class="form-label">AccNo</label>
                                    <input type="text" class="form-control" value="{{ $payment->accno ?? '-' }}" readonly>
                                </div>

                                <div class="col-md-4">
                                    <label class="form-label">@lang('form.status')</label>
                                    <input type="text"
                                        class="form-control {{ ($payment->Expr1 ?? '') == 'ชำระแล้ว' ? 'text-green-600' : 'text-red-600' }}"
