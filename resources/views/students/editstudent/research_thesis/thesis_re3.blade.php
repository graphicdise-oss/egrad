    ...
    <div class="flex min-h-screen">
        ...
        <div class="p-8">
            ...
            <div class="bg-white p-6 shadow-md w-3/4 border">
                ...
                <h2 class="text-lg font-semibold mb-4">@lang('form.thesis_re3_1')
                    <hr>
                </h2>
                <form method="POST"
                    action="{{ route('student.section.save', ['section' => 'thesis_re3', 'id_no' => $student->id_no]) }}">
                    @csrf
                        ...
                        <div class="w-3/4 flex space-x-2 items-center">
                            <div class="w-full"> <!-- ✅ แก้ตรงนี้: ให้ขยายเต็ม -->
                                <div
                                   ...
                                   <select name="tool_trial" class="text-sm form-select">
                                        <option value="">@lang('form.select')</option>
                                      
                                        <option value="in_progress" {{ ($student->tool_trial ?? '') == 'in_progress' ? 'selected' : '' }}>
                                            @lang('form.อยู่ในระหว่างดำเนินการ')
                                        </option>
                                        <option value="in_progress2" {{ ($student->tool_trial ?? '') == 'in_progress2' ? 'selected' : '' }}>
                                            @lang('form.ดำเนินการเรียบร้อยแล้ว')
                                        </option>
                                        <option value="not_progress" {{ ($student->tool_trial ?? '') == 'not_progress' ? 'selected' : '' }}>
                                            @lang('form.ไม่ต้องทดลองใช้เครื่องมือวิจัย')
                                        </option>
