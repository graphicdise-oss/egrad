    ...
    <div class="flex min-h-screen">
        ...
        <div class="p-8">
            ...
            <div class="bg-white p-6 shadow-md w-3/4 border">
                ...
                <h2 class="text-lg font-semibold mb-4">@lang('form.thesis_re6_1')
                    <hr>
                </h2>
                <form method="POST"
                    action="{{ route('student.section.save', ['section' => 'thesis_re6', 'id_no' => $student->id_no]) }}">
                    @csrf
                        ...
                        <div class="w-3/4 flex space-x-2 items-center">
                            <div class="w-full"> <!-- ✅ แก้ตรงนี้: ให้ขยายเต็ม -->
                                <div class="flex items-center  rounded-md shadow-sm overflow-hidden bg-gray-50">
                                    ...
                                    <!-- Select -->
                                        <select name="final_correction" class="text-sm form-select">
                                        <option value="">@lang('form.select')</option>
                                        
                                        <option value="in_progress" {{ ($student->final_correction ?? '') == 'in_progress' ? 'selected' : '' }}>
                                            @lang('form.อยู่ในระหว่างดำเนินการ')
                                        </option>
                                        <option value="in_progress2" {{ ($student->final_correction ?? '') == 'in_progress2' ? 'selected' : '' }}>
                            ...
                            <div class="w-full"> <!-- ✅ แก้ตรงนี้: ให้ขยายเต็ม -->
                                <div
                                    ...
                                    <input type="date" name="final_date"
                                        value="{{ old('approval_date', $student->final_date ?? '') }}"
                    ...
                    <div class="flex items-center space-x-4 md-4 mt-8">
                        ...
                        <div class="w-3/4 flex space-x-2">
                            ...
                            @php
                                $subject = "อัปเดทข้อมูลการรายงานการแก้ไขปรับปรุงวิทยานิพนธ์/การค้นคว้าอิสระะ";
                                $fullName = trim(preg_replace('/\s+/', ' ', "{$student->pname} {$student->name} {$student->lname}")); // ✅ รวมชื่อให้เหลือช่องว่างเดียว

                                $body = "เรียนคุณ {$fullName}\n\n"
                                    . "ได้มีการอัปเดทข้อมูลการรายงานการแก้ไขปรับปรุงวิทยานิพนธ์/การค้นคว้าอิสระ"
                                    . "กรุณาเข้าสู่ระบบ E-Graduate เพื่อดำเนินการตรวจสอบข้อมูลเพิ่มเติมได้ที่ลิงก์ด้านล่าง\n\n"
                                    . "https://egrad.vru.ac.th/gradstd/login\n\n"
                                    . "---------------------------------------------\n\n"
                                    . "Dear {$fullName}\n\n"
                                    . "The information regarding the submission of the Thesis/Independent Study revisions has been updated."
