    ...
    <div class="flex min-h-screen">
        ...
        <div class="p-8">
            ...
            <div class="bg-white p-6 shadow-md w-3/4 border">
                ...
                <h2 class="text-lg font-semibold mb-4">@lang('form.sys_fi3_1')
                    <hr>
                </h2>
                <form method="POST"
                    action="{{ route('student.section.save', ['section' => 'sys_fi3', 'id_no' => $student->id_no]) }}">
                    @csrf
                        ...
                        <div class="w-3/4 flex space-x-2 items-center">
                            <div class="w-full"> <!-- ✅ แก้ตรงนี้: ให้ขยายเต็ม -->
                                <div
                                    ...
                                    <!-- Select -->
                                       <select name="sys_fi3" class="text-sm form-select">
                                        <option value="">@lang('form.select')</option>
                                     
                                        <option value="in_progress" {{ ($student->sys_fi3 ?? '') == 'in_progress' ? 'selected' : '' }}>
                                            @lang('form.อยู่ในระหว่างดำเนินการ')
                                        </option>
                                        <option value="in_progress2" {{ ($student->sys_fi3 ?? '') == 'in_progress2' ? 'selected' : '' }}>
                    ...
                    <div class="flex items-center space-x-4 mb-6">
                        ...
                        <div class="w-3/4 flex space-x-4 items-center">
                            ...
                            <!-- ช่องครั้งที่ -->
                            <div class="w-1/3">
                                <input type="text" name="sys_fi3_time"
                                    class="w-full border border-gray-500 p-2 text-sm rounded-md bg-gray-50 shadow-sm"
                                    placeholder="ครั้งที่" value="{{ $student->sys_fi3_time ?? '' }}">
                            </div>
                           

                            <!-- ช่องวันที่ -->
                            <div class="w-2/3">
                               <input type="date" name="sys_fi3_date"
                                    value="{{ old('sys_fi3_date', $student->sys_fi3_date ?? '') }}"
                                    class="w-full border border-gray-500 p-2 text-sm rounded-md bg-gray-50 shadow-sm">
                            </div>
