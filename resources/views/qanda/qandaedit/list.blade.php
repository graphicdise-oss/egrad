...
">
            ...
            <form method="GET" action="{{ url('/qanda/edit') }}" class="mb-4">
                    ...
                    <div class="col-md-4">
                        <div class="input-icon-wrapper" style="position: relative;">
                            ...
                            <select name="type" class="custom-select-qanda" onchange="this.form.submit()"
                                ...
                                <option value="ร้องเรียน" {{ request('type') == 'ร้องเรียน' ? 'selected' : '' }}>ร้องเรียน
                                </option>
                                <option value="แนะนำติชม" {{ request('type') == 'แนะนำติชม' ? 'selected' : '' }}>
                                    แนะนำติชม</option>
                                <option value="สอบถามข้อมูล" {{ request('type') == 'สอบถามข้อมูล' ? 'selected' : '' }}>
                                    สอบถามข้อมูล</option>
                                <option value="ยังไม่ได้ตอบ" {{ request('type') == 'ยังไม่ได้ตอบ' ? 'selected' : '' }}>
                                    ยังไม่ได้ตอบ</option>
                            </select>
                        </div>
                    </div>
                ...
                @foreach ($qa as $item)
                    <div style="border-bottom: 1px solid #ccc; padding: 10px;">
                        ...
                        <div style="margin-top: 8px;">
                            <a href="{{ route('qanda.edit', $item->id) }}"
                                style="display: inline-block; padding: 4px 10px; background-color: #0d6efd; color: white; border-radius: 6px; text-decoration: none; font-size: 20px;">
                                ตอบคำถาม
                            </a>

                            <form action="{{ route('qanda.destroy', $item->id) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('คุณแน่ใจว่าต้องการลบคำถามนี้?')"
                                    style="padding: 4px 10px; background-color: red; color: white; border: none; border-radius: 6px; font-size: 20px; cursor: pointer;">
                                    ลบ
                                </button>
                            </form>
                        </div>
