    ...
    <div class="flex min-h-screen">
        ...
        <div class="p-8">
            ...
            <div class="bg-white p-6 shadow-md w-3/4 border">
                ...
                <h2 class="text-lg font-semibold mb-4">@lang('form.การตรวจสอบเครื่องมือที่ใช้ในงานวิจัย')
                    <hr>
                </h2>

                <form method="POST"
                    action="{{ route('student.section.save', ['section' => 'thesis_re1', 'id_no' => $student->id_no]) }}">
                    @csrf
                        ...
                        <div class="w-3/4 flex space-x-2 items-center">
                            <div class="w-full"> <!-- ✅ แก้ตรงนี้: ให้ขยายเต็ม -->
                                <div
                                    ...
                                    <!-- Select -->
                                    <select name="tool_check" class="text-sm form-select">
                                        <option value="">@lang('form.กรุณาเลือกข้อมูล')</option>
                                        <option value="not_started" {{ ($student->tool_check ?? '') == 'not_started' ? 'selected' : '' }}>
                                            @lang('form.ยังไม่ได้ดำเนินการ')
                                        </option>
                                        <option value="in_progress" {{ ($student->tool_check ?? '') == 'in_progress' ? 'selected' : '' }}>
                                            @lang('form.อยู่ในระหว่างดำเนินการ')
                                        </option>
                                        <option value="completed" {{ ($student->tool_check ?? '') == 'completed' ? 'selected' : '' }}>
                                            @lang('form.ดำเนินการเรียบร้อยแล้ว')
                                        </option>
                                        <option value="not_applicable" {{ ($student->tool_check ?? '') == 'not_applicable' ? 'selected' : '' }}>
                                            @lang('form.ไม่ต้องตรวจสอบเครื่องมือที่ใช้ในการวิจัย')
                                        </option>
                                    </select>
                    ...
                    <div class="flex items-start space-x-4 mb-6">
                        ...
                        <div class="w-3/4">
                            <!-- ปุ่มดูเอกสาร -->
                            <a href="{{ asset('pdfs/m_w_r_b/form_5.pdf') }}" target="_blank"
                    ...
                    <div class="flex items-center space-x-4 md-4 mt-8">
                        ...
                        <div class="w-3/4 flex space-x-2">
                            ...
                            @php
                                $subject = "อัปเดตข้อมูลการตรวจสอบเครื่องมือที่ใช้ในงานวิจัยะ";
                                ...
                                $body = " {$fullName}\n\n รหัสนักศึกษา {$student->id_no}"
                                    . " ได้มีการอัปเดทข้อมูลการตรวจสอบเครื่องมือที่ใช้ในงานวิจัย "
                                    . "กรุณาเข้าสู่ระบบ E-Graduate เพื่อดำเนินการตรวจสอบข้อมูลเพิ่มเติมได้ที่ลิงก์ด้านล่าง\n\n"
                                    . "https://egrad.vru.ac.th/section/thesis_re1/{$student->id_no}\n\n"
                                    . "---------------------------------------------\n\n";
